...
struct AES {
    ...
    func encrypt(string: String) -> String? {
        if let _d = crypt(data: string.data(using: .utf8), option: CCOperation(kCCEncrypt)) {
            let base64Data = _d.base64EncodedData()
...
let key256 = "11111178901234561234567890111111" // 32 bytes for AES256
let iv = "abcdefghijklmnop"

extension AES {
    static var shared = AES(key: key256, iv: "abcdefghijklmnop")
}
